...
import { Mail, Lock, Eye, EyeOff, User, Loader2, ArrowRight, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      <AnimatePresence mode="wait">
        {/* ===== BUTTONS MODE ===== */}
        {mode === MODES.BUTTONS && (
          <motion.div key="buttons" variants={formVariants} initial="hidden" animate="visible" exit="exit" className="space-y-3">
            <button
            ...
            <div className="text-center pt-2">
              <button onClick={() => setMode(MODES.SIGNUP)} className="text-sm text-gray-500 hover:text-red-600 transition-colors">
        ...
        {mode === MODES.EMAIL_LOGIN && (
          <motion.form key="email_login" variants={formVariants} initial="hidden" animate="visible" exit="exit" onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <Button type="submit" disabled={isLoading} className="w-full h-12 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold">
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'התחברות'}
            </Button>

            <div className="flex items-center justify-between text-sm">
              <button type="button" onClick={() => setMode(MODES.FORGOT)} className="text-gray-500 hover:text-red-600 transition-colors">
                שכחתי סיסמה
              </button>
              <button type="button" onClick={() => { setMode(MODES.BUTTONS); setError(''); }} className="flex items-center gap-1 text-gray-500 hover:text-gray-700 transition-colors">
        ...
        {mode === MODES.SIGNUP && (
          <motion.form key="signup" variants={formVariants} initial="hidden" animate="visible" exit="exit" onSubmit={handleSignup} className="space-y-4">
            ...
            <div className="text-center">
              <button type="button" onClick={() => { setMode(MODES.BUTTONS); setError(''); }} className="flex items-center gap-1 mx-auto text-sm text-gray-500 hover:text-gray-700 transition-colors">
                <ChevronRight className="w-4 h-4 icon-flip-rtl" />
        ...
        {mode === MODES.FORGOT && (
          <motion.form key="forgot" variants={formVariants} initial="hidden" animate="visible" exit="exit" onSubmit={handleForgotPassword} className="space-y-4">
            <p className="text-sm text-gray-600 text-right mb-2">הזינו את כתובת האימייל ונשלח לכם קישור לאיפוס הסיסמה</p>
            <div className="relative">
              <Mail className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
              <input
                type="email"
            ...
            {error && <p className="text-red-500 text-sm text-right">{error}</p>}
            {forgotSuccess && <p className="text-green-600 text-sm text-right">קישור לאיפוס סיסמה נשלח לאימייל שלך</p>}
            ...
            <div className="text-center">
              <button type="button" onClick={() => { setMode(MODES.EMAIL_LOGIN); setError(''); setForgotSuccess(false); }} className="flex items-center gap-1 mx-auto text-sm text-gray-500 hover:text-gray-700 transition-colors">
                <ChevronRight className="w-4 h-4 icon-flip-rtl" />
                חזרה להתחברות
              </button>
            </div>
