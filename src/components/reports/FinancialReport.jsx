...
export default function FinancialReport({ calls }) {
  ...
  const stats = useMemo(() => {
    ...
    const chargedDeposits = deposits
      .filter((d) => d.status === 'charged')
      .reduce((sum, d) => sum + (d.charged_amount || 0), 0);
  ...
  const paymentMethodData = useMemo(() => {
    ...
    calls.forEach((c) => {
      if (c.payment_type) counts[c.payment_type] = (counts[c.payment_type] || 0) + 1;
    });
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">עלויות ספקים</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">מכירת מוצרים</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold text-red-600">
              ₪{stats.chargedDeposits.toLocaleString()}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold text-gray-800">
              ₪{stats.vendorTotal.toLocaleString()}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">סטטוס עירבונות</CardTitle>
          </CardHeader>
          <CardContent>
