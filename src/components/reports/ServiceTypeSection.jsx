...
export default function ServiceTypeSection({ cases }) {
  ...
  const exportData = rows.map(r => ({
    'סוג שירות': r.name, 'סה"כ': r.total, '%': r.pct, 'הכנסות': r.revenue, 'עלות ממוצעת': r.avgCost,
  }));
      ...
      <CardContent className="space-y-4">
        <div className="grid md:grid-cols-2 gap-6">
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={rows} dataKey="total" nameKey="name" cx="50%" cy="50%" outerRadius={100} label={({ name, pct }) => `${name} ${pct}%`} labelLine={false}>
