...
import { MONTHS_HE, fmtNum, fmtCurrency, getMonth, getEffectiveDate } from './ReportUtils';
...
export default function MonthlyTrendSection({ cases }) {
  const byMonth = Array.from({ length: 12 }, (_, i) => {
    ...
    const avgCost =
      mCases.length > 0 ? mCases.reduce((s, c) => s + (c.cost || 0), 0) / mCases.length : 0;
    const avgKm =
      mCases.length > 0 ? mCases.reduce((s, c) => s + (c.distance_km || 0), 0) / mCases.length : 0;
    return {
      month: MONTHS_HE[i],
      monthIdx: i,
      total: mCases.length,
  ...
  const exportData = withMom.map((m) => ({
    ...
    'ממוצע נע 3M': m.ma3,
    הכנסות: m.revenue,
    'עלות ממוצעת': m.avgCost.toFixed(0),
    'ק"מ ממוצע': m.avgKm.toFixed(1),
  }));
            ...
            <tbody>
              {withMom.map((m) => (
                <tr key={m.month} className="border-b hover:bg-gray-50">
                  ...
                  <td
                    className={`px-3 py-2 ${m.mom > 0 ? 'text-green-600' : m.mom < 0 ? 'text-red-600' : ''}`}
                  >
