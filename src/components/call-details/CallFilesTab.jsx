...
import { Camera, FileText, Trash2, Pencil, Eye, Download, Image as ImageIcon, FolderOpen } from 'lucide-react';
  ...
  return (
    <div className="relative group border rounded-lg overflow-hidden bg-white">
      ...
      <div className="absolute top-1 left-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button size="icon" variant="destructive" className="h-7 w-7" onClick={() => onDelete(photo)}>
      ...
      <div className="p-2">
        ...
        <p className="text-[10px] text-gray-300 mt-0.5">
          {photo.created_date ? format(new Date(photo.created_date), 'dd/MM/yy HH:mm', { locale: he }) : ''}
...
export default function CallFilesTab({ callId, photos, onFilesUploaded }) {
  ...
  const activeDocuments = activePhotos.filter(p => !p.file_url?.match(/\.(jpg|jpeg|png|gif|webp)$/i));
      ...
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          {activePhotos.length > 0 && (
            <Badge variant="outline" className="text-xs">{activePhotos.length} קבצים</Badge>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
            ...
            <TabsContent value="all">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
                {activePhotos.map((photo) => (
                  <PhotoCard key={photo.id} photo={photo} onEdit={handleEdit} onDelete={handleDelete} />
                ))}
              </div>
            </TabsContent>
        ...
        <Dialog open={!!editPhoto} onOpenChange={() => setEditPhoto(null)}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setEditPhoto(null)}>ביטול</Button>
              <Button onClick={handleSaveEdit} isLoading={saving}>שמור</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </CardContent>
