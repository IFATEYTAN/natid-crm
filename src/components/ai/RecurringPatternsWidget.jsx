...
import { Sparkles, Loader2, RefreshCw, User, Car, MapPin, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';

const issueLabels = {
  mechanical: 'מכנית', stopped_driving: 'לא נוסע', flat_tire: "פנצ'ר",
  stuck_wheel: 'גלגל תקוע', accident: 'תאונה', no_fuel: 'דלק',
  dead_battery: 'מצבר', locked_keys: 'מפתחות', other: 'אחר'
};
...
export default function RecurringPatternsWidget() {
  ...
  const analyze = async () => {
    setLoading(true);
    const response = await base44.functions.invoke('analyzeHistoricalPatterns', { analysis_type: 'recurring_patterns' });
    ...
    <Card className="border-purple-200">
      ...
      {result && !loading && expanded && (
        <CardContent className="space-y-4">
          ...
          {result.ai_patterns?.length > 0 && (
            <div className="space-y-2">
              ...
              {result.ai_patterns.map((pattern, i) => (
                <div key={i} className={`p-3 rounded-lg border ${severityColors[pattern.severity] || severityColors.info}`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-sm">{pattern.title}</span>
                    <Badge variant="outline" className="text-[10px]">{severityLabels[pattern.severity]}</Badge>
                  </div>
            ...
            {result.repeating_customers?.length > 0 && (
              <div className="space-y-2">
                ...
                {result.repeating_customers.slice(0, 5).map((c, i) => (
                  <div key={i} className="bg-white rounded-lg p-2.5 border text-sm flex justify-between items-center">
                    <div>
                      <span className="font-medium">{c.name}</span>
                      <span className="text-xs text-gray-500 mr-2">{c.phone}</span>
                    </div>
                    <Badge variant="secondary" className="text-xs">{c.total_calls} קריאות</Badge>
                  </div>
                ))}
              </div>
            )}
            ...
            {result.repeating_vehicles?.length > 0 && (
              <div className="space-y-2">
                ...
                {result.repeating_vehicles.slice(0, 5).map((v, i) => (
                  <div key={i} className="bg-white rounded-lg p-2.5 border text-sm flex justify-between items-center">
                    <div>
                      <span className="font-medium" dir="ltr">{v.plate}</span>
                      <span className="text-xs text-gray-500 mr-2">{v.model}</span>
                    </div>
                    <Badge variant="secondary" className="text-xs">{v.total_calls} קריאות</Badge>
                  </div>
                ))}
              </div>
            )}
          ...
          <p className="text-[11px] text-gray-400 text-center">{result.total_analyzed} קריאות נותחו</p>
        </CardContent>
      )}
