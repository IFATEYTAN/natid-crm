...
export default function VendorRecommendation({ callDetails, onSelectVendor }) {
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => base44.functions.invoke('recommendVendor', { call_details: callDetails }).then(res => res.data),
        enabled: !!callDetails,
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                                    ...
                                    <span className="flex items-center gap-1">
                                        ...
                                        {rec.vendor.average_response_time ? `~${Math.round(rec.vendor.average_response_time)} דק'` : '-'}
                                    </span>
                                </div>
                            </div>
                            <Badge variant="outline" className={`
                                ${rec.match_score >= 90 ? 'bg-green-50 text-green-700 border-green-200' : 
                                  rec.match_score >= 75 ? 'bg-blue-50 text-blue-700 border-blue-200' : 
