...
export default function FormSelect({
  ...
  return (
    <div className="space-y-2">
      {label && (
        <Label className="block text-end text-[#212121] font-medium text-[14px]">
          {label}
          {required && <span className="text-[#D32F2F] me-1 text-base">*</span>}
        </Label>
      )}
      <Select {...props}>
        <SelectTrigger
          className={cn(
            'border border-[#E0E0E0] rounded-[4px] px-4 py-3 text-base text-end bg-white h-auto',
      ...
      {helperText && !error && (
        <p className="text-[#616161] text-xs caption text-end">{helperText}</p>
      )}
    </div>
  );
}
