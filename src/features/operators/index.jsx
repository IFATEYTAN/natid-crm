...
export default function OperatorDashboard() {
  ...
  const stats = {
    ...
    urgent: openCalls.filter((c) => c.call_priority === 'urgent' || c.call_priority === 'critical')
      .length,
  ...
  const callColumns = [
    {
      ...
      cell: (row) => (
        <Link
          ...
          className="font-semibold text-[#212121] hover:text-[#FF0000]"
        >
      ...
      cell: (row) => (
        <div>
          ...
          <a
            ...
            className="text-xs text-[#616161] flex items-center gap-1"
          >
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-[#616161]">
          {row.pickup_location_city || row.pickup_location_address?.substring(0, 30)}
        </span>
      ),
    },
    {
  ...
  const vendorColumns = [
    {
      ...
      cell: (row) => (
        <Link
          ...
          className="font-medium text-[#212121] hover:text-[#FF0000] flex items-center gap-2"
        >
      ...
      cell: (row) => (
        <div className="flex flex-wrap gap-1">
          {(row.coverage_areas || []).slice(0, 2).map((area, idx) => (
            <span
              key={idx}
              className="text-xs bg-[#FAFAFA] text-[#616161] px-2 py-1 rounded border border-[#E0E0E0]"
            >
              {area}
            </span>
          ))}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-lg p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Link to={createPageUrl('NewCase')}>
            <Button
              variant="outline"
              className="w-full h-20 flex flex-col gap-2 border border-[#E5E7EB] hover:border-[#D1D5DB] hover:bg-[#F9FAFB]"
            >
              ...
              >
                <div className="flex-1">
                  <Link
                    ...
                    className="font-medium text-[#111827] hover:text-[#DC2626]"
                  >
                    {call.call_number || `#${call.id?.slice(-6)}`}
                  </Link>
                  <p className="text-[13px] text-[#6B7280]">
                    {call.customer_name} - {call.pickup_location_city}
                  </p>
                </div>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-lg p-5">
        ...
        <DataTable
          ...
          onRowClick={(row) =>
            (window.location.href = createPageUrl('CaseDetails') + '?id=' + row.id)
          }
