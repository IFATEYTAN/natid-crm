...
export default function OperationalRatesPage() {
  ...
  const handleSave = async () => {
    if (!form.name || !form.amount) return toast.error('יש למלא שם וסכום');
    setSaving(true);
    const data = {
      ...form,
    ...
    setShowDialog(false);
    setSaving(false);
    toast.success(editingRate ? 'תעריף עודכן' : 'תעריף נוצר');
  };

  const handleDelete = async (id) => {
    await base44.entities.OperationalRate.delete(id);
    queryClient.invalidateQueries({ queryKey: queryKeys.operationalRates.all() });
    toast.success('תעריף נמחק');
  };
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-gray-500 flex items-center gap-2 flex-row-reverse justify-end">
                          ...
                          {rate.applies_from_hour != null && (
                            <span>
                              שעות: {rate.applies_from_hour}:00-{rate.applies_to_hour}:00
                            </span>
                          )}
                          {rate.applies_to_areas?.length > 0 && (
                            <span>
                              אזורים:{' '}
                              {rate.applies_to_areas
                                .map((a) => areaOptions.find((o) => o.value === a)?.label || a)
                                .join(', ')}
                            </span>
                          )}
                        </div>
              ...
              <div className="flex items-end gap-3 pb-1">
                <div className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={(v) => setForm({ ...form, is_percentage: v })}
                  />
                  <Label className="mb-0">
                    {form.is_percentage ? 'אחוז (%)' : 'סכום קבוע (₪)'}
                  </Label>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    min={0}
                    max={23}
                    value={form.applies_from_hour}
                    onChange={(e) => setForm({ ...form, applies_from_hour: e.target.value })}
                    placeholder="0"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    min={0}
                    max={23}
                    value={form.applies_to_hour}
                    onChange={(e) => setForm({ ...form, applies_to_hour: e.target.value })}
                    placeholder="24"
                  />
                </div>
              </div>
