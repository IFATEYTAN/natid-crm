...
export default function VendorPortalPage() {
    ...
    queryFn: async () => {
      ...
      if (isVendorUser) {
        const result = await base44.functions.invoke('getVendorScopedData', {
          entity_type: 'profile', sort: '-created_date', limit: 1,
        });
  ...
  const callsQuery = useQuery({
    ...
    queryFn: async () => {
      if (isVendorUser) {
        const result = await base44.functions.invoke('getVendorScopedData', {
          entity_type: 'calls', sort: '-created_date', limit: 1000,
        });
  ...
  const contractQuery = useQuery({
    ...
    queryFn: async () => {
      if (isVendorUser) {
        const result = await base44.functions.invoke('getVendorScopedData', {
          entity_type: 'contracts', sort: '-created_date', limit: 10,
        });
      ...
      const contracts = await base44.entities.VendorContract.filter(
        { vendor_id: vendorProfile.id, status: 'active' }, '-created_date', 1
      );
  ...
  const pendingAssignmentsQuery = useQuery({
    ...
    queryFn: async () => {
      if (isVendorUser) {
        const result = await base44.functions.invoke('getVendorScopedData', {
          entity_type: 'attempts', sort: '-created_date', limit: 10,
        });
      ...
      return base44.entities.CallAssignmentAttempt.filter(
        { vendor_id: vendorProfile.id, status: 'pending' }, '-created_date', 1
      );
    },
  ...
  const updateCallStatus = async (callId, newStatus) => {
    ...
    if (newStatus === 'in_progress') updateData.vendor_arrival_time_actual = new Date().toISOString();
            ...
            <>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  {isAdmin && <p className="text-xs text-[#6B778C] mt-1">מציג בתור ספק (צפייה כאדמין)</p>}
              ...
              <Suspense fallback={<div className="h-16" />}>
                <VendorAvailabilityToggleLazy vendor={vendorProfile} isAvailable={isAvailable} onToggle={toggleAvailability} />
              </Suspense>
