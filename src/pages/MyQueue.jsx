...
export default function MyQueuePage() {
  ...
  const { data: allCalls = [], isLoading, refetch, isFetching } = useQuery({
  ...
  const myCalls = useMemo(() => {
    ...
    return allCalls.filter(call => 
      call.created_by === currentUser.email
    );
  ...
  const completedCalls = myCalls.filter(c => 
  ...
  const stats = useMemo(() => ({
    ...
    urgent: myCalls.filter(c => c.call_priority === 'urgent' || c.call_priority === 'critical').length,
    ...
    },
    {
      ...
      cell: (call) => (
        <Badge className={cn("text-xs", priorityColors[call.call_priority] || priorityColors.normal)}>
          {call.call_priority === 'urgent' ? 'דחוף' : call.call_priority === 'critical' ? 'קריטי' : 'רגיל'}
        </Badge>
      )
    },
    {
