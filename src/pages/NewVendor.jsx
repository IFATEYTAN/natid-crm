...
export default function NewVendorPage() {
  ...
  // Check for edit mode (id query param)
  ...
  // Load existing vendor data in edit mode
  const vendorQuery = useQuery({
    queryKey: ['vendorEdit', editId],
    queryFn: async () => {
      const vendors = await base44.entities.Vendor.filter({ id: editId });
  ...
  useEffect(() => {
    if (vendorQuery.data) {
      ...
      setFormData({
        ...
        vehicle_types_supported: v.vehicle_types_supported || [],
        coverage_areas: v.coverage_areas || [],
        coverage_cities: v.coverage_cities || '',
        works_24_7: v.works_24_7 || false,
        working_hours_start: v.working_hours_start || '08:00',
        working_hours_end: v.working_hours_end || '18:00',
        payment_rate_per_call: v.payment_rate_per_call || '',
        notes: v.notes || '',
        is_active: v.is_active !== false,
        is_available_now: v.is_available_now || false,
        availability_status: v.availability_status || 'offline',
      });
      setSendInvite(false);
    }
  }, [vendorQuery.data]);
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      if (isEditMode) {
        ...
        return base44.entities.Vendor.update(editId, data);
      }
      // Create vendor
    ...
    onSuccess: () => {
      showToast.success(
        isEditMode ? feedbackMessages.save.success : feedbackMessages.create.success
      );
      navigate(createPageUrl('ServiceProviders'));
    },
    onError: (error) => {
      showToast.error(
        (isEditMode ? feedbackMessages.save.error : feedbackMessages.create.error) +
          ': ' +
          error.message
      );
    },
  ...
  if (isEditMode && vendorQuery.isLoading) {
    return <PageLoader text="טוען ספק..." />;
  }
    ...
    <SlideUp>
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[#6B778C] text-sm">
              {isEditMode ? 'עדכון פרטי ספק קיים' : 'יצירת ספק שירות חדש והזמנתו למערכת'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Invite Section - only for new vendors */}
          {!isEditMode && (
            ...
            >
              {createMutation.isPending ? (
                ...
                <>
                  ...
                  {isEditMode ? 'שמור שינויים' : sendInvite ? 'צור ושלח הזמנה' : 'צור ספק'}
                </>
              )}
            </Button>
          </div>
