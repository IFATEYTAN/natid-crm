...
// In-memory sync-run status (Deno KV unavailable in this runtime; persists on warm starts).
function recordSyncRun(run) {
  natiLastSyncRun = run;
  return Promise.resolve();
}

function getNatiStatus() {
  const now = Date.now();
  const blocked = natiCircuit.blockedUntil > now;
  return Promise.resolve({
    lastRun: natiLastSyncRun,
    circuit: {
      blocked,
      blockedUntil: natiCircuit.blockedUntil,
      reason: natiCircuit.reason,
      retryAfterSec: blocked ? Math.ceil((natiCircuit.blockedUntil - now) / 1000) : 0,
    },
  });
}
