...
**תאריך:** 15/07/2026 (עדכון אחרון: פיילוט Claude API, תסריטי שיחה, דרישת MFA)
...
מאגר ידע תקלות/פתרונות עם חיפוש ותגיות, ותסריטי שיחה למוקדנים עם העתקה בלחיצה. תסריטי השיחה מסווגים לפי סוג (פתיחה / אבחון / הסלמה / סגירה / תלונה / שדרוג) ולפי סוג שירות, וניתנים לניהול מלא מהמסך. ישות `CallScript` הוגדרה ב-14/07 ונזרעו 7 תבניות בסיס (פתיחת שיחה, אבחון רכב-לא-מניע, אבחון גרירה, הסלמת המתנה, טיפול בתלונה, סגירת שיחה, שדרוג למנוי) — למוקד מומלץ להתאים ולהרחיב אותן.
...
מסך העבודה של הספק בקריאה: התקדמות סטטוס ("יצאתי לדרך" → "הגעתי" → סיום), ניווט Waze, פרטי לקוח ורכב, העלאת תמונות לפני/אחרי, **חתימת לקוח דיגיטלית (חובה לסגירה)**, צ'אט ישיר עם המוקד בזמן אמת, "לא ניתן להשלים", שחרור קריאה חזרה לתור.
...
### 5.4 מסע הספק צעד-אחר-צעד — פירוט מסכי המשנה

מסלול העבודה המלא של ספק בקריאה, כפי שממומש בפורטל (`/VendorPortal` → `/VendorCallManagement`):

**שלב 1 — קבלת ההצעה.** הספק מקבל במקביל: (א) SMS — "נתיד - קריאה חדשה #… לקוח… כתובת… סוג… היכנס לפורטל לאישור"; (ב) התראת מערכת בפעמון; (ג) חלון קופץ בפורטל (`VendorNewCallAlert`) עם פרטי הקריאה וספירה לאחור. ההצעה תקפה **10 דקות** — אחריהן היא עוברת אוטומטית לספק הבא.

**שלב 2 — קבלה / דחייה.** "קבל קריאה" → הקריאה עוברת ל"ספק בדרך", הספק מסומן עסוק, והמוקד מקבל התראה. "דחה" → נדרשת סיבת דחייה, וההצעה עוברת לספק הבא בניקוד.

**שלב 3 — מסך ניהול הקריאה** (`/VendorCallManagement`) — רכיבי המשנה:

| רכיב | מה הספק עושה בו |
|---|---|
| פס התקדמות סטטוס (`VendorCallStatusProgress`) | רואה את השלב הנוכחי במסלול: בדרך → הגיע → בטיפול → סגירה |
| סרגל פעולות (`VendorCallActionBar`) | כפתורי הפעולה הראשיים: **"הגעתי למקום"** (מעביר ל"בטיפול" + הודעה אוטומטית בצ'אט "הספק הגיע למקום ומתחיל בטיפול") ו-**"סיום קריאה"** |
| ניווט | פתיחת Waze ישירות לכתובת האירוע |
| פרטי לקוח ורכב (`VendorCallCustomerInfo`, `VendorCallVehicleInfo`) | שם, טלפון עם חיוג בלחיצה, כתובת, פרטי רכב (מספר, דגם, סוג) |
| ציר הזמן של הלקוח (`VendorCustomerJourney`) | תצוגת השלבים שהלקוח רואה — מה שוקף ללקוח בכל רגע |
| **העלאת תמונות** (`VendorPhotoUploader`) | צילום/העלאה מהאירוע בשתי קטגוריות מובנות: **"לפני טיפול"** ו-**"אחרי טיפול"**; תצוגה מקדימה, מחיקה רכה (`is_deleted`). כל תמונה נשמרת כ-`CallPhoto` משויכת לקריאה |
| **חילוץ AI מתמונות** (`VendorPhotoAIExtractor`) | ניתוח אוטומטי של התמונה ומילוי `ai_extracted_data` (למשל זיהוי פרטי רכב/נזק) |
| **חתימת לקוח** (`SignaturePad`) | הלקוח חותם על מסך המגע; החתימה נשמרת כתמונת PNG בקטגוריה `customer_signature` עם טביעת אצבע SHA-256 בשם הקובץ (לאימות שלמות). **בלי חתימה — כפתור הסיום חסום** (בדיקת `hasSignature`) |
| **צ'אט מול המוקד** (`EnhancedCallChat`) | הודעות דו-כיווניות עם המוקדן **בזמן אמת** (subscription על ישות Message; רענון 30 שניות כגיבוי); עדכוני הסטטוס של הספק מתועדים בצ'אט אוטומטית |
| טופס משוב (`CallFeedbackForm`) | דיווח הספק בסיום |
| "לא ניתן להשלים" | הקריאה נשארת אצל הספק בסטטוס `cannot_complete` והמוקד מקבל התראה לטיפול |
| שחרור קריאה | החזרת הקריאה לתור השיבוץ (`releaseVendorCall`) |

**שלב 4 — GPS לאורך כל הקריאה.** כל עוד הקריאה פעילה, `VendorGPSTracker` משדר מיקום (כולל ברקע באפליקציה הנייטיבית); אם המיקום מתיישן — הספק מקבל push "נדרש עדכון מיקום" (`nudgeStaleVendorLocations`).

**שלב 5 — סיום.** "סיום קריאה" (לאחר חתימה) → הקריאה נסגרת, הודעת "הטיפול סגור בהצלחה!" מתועדת בצ'אט, הספק חוזר לזמין, והלקוח מקבל SMS סיום + סקר משוב. אם נבחר סטטוס סגירה שמחייב המשך (למשל "ניידת לא צלחה — יש לשלוח גרר") — נוצרת **קריאת המשך** מקושרת אוטומטית.

### 5.5 תבניות ההודעות והטריגרים שלהן

כל ההודעות נשלחות **אוטומטית על ידי המערכת** בתגובה לשינויי סטטוס — הספק לא שולח SMS ללקוח ידנית. ערוצים: SMS (Twilio), WhatsApp (Twilio/Green API — עם קידומת "נתיב: "), התראת מערכת, אימייל (עם חתימת "מערכת נתיב - שירותי דרך").

**הודעות ללקוח** (`sendCallStatusUpdate` — נשלחות בשינוי סטטוס; ניתן גם `custom_message` חופשי):

| טריגר (סטטוס) | נוסח נוכחי |
|---|---|
| ממתין לשיבוץ | "הקריאה התקבלה ואנחנו מחפשים ספק זמין" |
| שובץ ספק | "מצאנו ספק מתאים ומחכים לאישורו" |
| ספק בדרך | "הספק בדרך אליך! צפי הגעה: {eta}" |
| בטיפול | "הספק הגיע ומתחיל בטיפול" |
| הושלם | "הטיפול הושלם בהצלחה. תודה שבחרת בנו!" |
| כל סטטוס אחר | "סטטוס הקריאה עודכן: {סטטוס}" |

**הודעות בסגירת קריאה** (`closingStatuses`): 6 מתוך 7 סטטוסי הסגירה שולחים SMS ללקוח (גרר לאחסנה — לא שולח). ⚠️ **כל ששת הנוסחים הם כרגע placeholder** ("נוסח SMS זמני — להחלפה בנוסח הסופי מהלקוח") — ממתינים לנוסחים סופיים מהלקוח. זו משימה פתוחה מול דורית.

**סקר משוב** (`sendFeedbackSMS`, לאחר סגירה): "שלום {שם פרטי}, תודה שבחרת בנתי! נשמח אם תדרג את השירות שקיבלת: {קישור}" — הקישור הוא token חד-פעמי עם תפוגה לטופס `/CustomerFeedback`.

**הודעות לספק:** הצעת שיבוץ — SMS "נתיד - קריאה חדשה #{מספר} / לקוח / כתובת / סוג / היכנס לפורטל לאישור" (עם מניעת כפילות בחלון 10 דקות) + התראת מערכת "הצעת קריאה חדשה"; push "נדרש עדכון מיקום" כשה-GPS מתיישן.

**הודעות למוקד:** קבלה/דחייה של ספק, הסלמת קריאה ללא מענה ("🚨 קריאה דורשת שיבוץ ידני"), "לא ניתן להשלים", והתראות חכמות (`detectSmartAlerts`).

> תזכורת: משלוח בפועל מותנה בהפעלת חשבון Twilio production (פרק 9.2). הלוגיקה, הטריגרים והתיעוד ב-`CallHistory`/`SmsHistory` — ממומשים במלואם.
...
### 6.1 עיקרון
נתי (MySQL על AWS RDS il-central-1) היא **מקור האמת** לנתוני הפתיחה והדיווח. הסנכרון **דו-כיווני**: משיכה נתי → CRM (`syncNatiData`) ודחיפה CRM → נתי (`pushNatiUpdates`, נוסף 15/07 לאחר קבלת הרשאות כתיבה). הדחיפה שמרנית: סטטוס נע קדימה בלבד (שיבוץ 0→1, סגירה/ביטול), ושדות שיבוץ/זמנים/הערות/QC נכתבים רק כשהם ריקים בנתי — נתי לעולם לא נדרסת. נכון ל-14/07/2026: **סנכרון מלא 155/155 קריאות פתוחות אחד-לאחד**, כולל קישור ספקים ולקוחות.
...
| כתיבה חזרה לנתי (דו-כיווני) | ✅ ממומש (`pushNatiUpdates`, 15/07) — דורש הגדרת תזמון בלוח Base44 |
...
| 8 | **אין אימות דו-שלבי (MFA)** — דרישה עסקית פתוחה (בקשת אורטל) | 🟠 | ראו אפיון מלא ב-13.3 |

### 13.3 דרישה פתוחה — אימות דו-שלבי (MFA) [בקשת אורטל]

**מצב נוכחי:** ההתחברות למערכת מנוהלת כולה על ידי פלטפורמת Base44 (`base44.auth.redirectToLogin`) — אימייל + סיסמה בלבד, ללא גורם אימות שני. אין שום מימוש MFA/OTP בקוד האפליקציה (רכיב ה-UI `input-otp` קיים בספריית הרכיבים אך אינו בשימוש).

**הדרישה (אורטל):** קוד OTP **בכל כניסה לממשק**, בנוסף לאימייל+סיסמה — לכל הפחות למשתמשי admin (ורצוי לכלל התפקידים — המערכת מכילה פרטי לקוחות, טלפונים וכתובות).

**בדיקת פלטפורמה (בוצעה 15/07 מול מסך Settings → Authentication ב-Base44):** אין מתג MFA/OTP מובנה. האפשרויות בפלטפורמה: אימייל+סיסמה (פעיל כיום), ספקי התחברות חברתיים (Google/Microsoft/Facebook/Apple — כבויים), ו-SSO מותאם **בחבילת Elite בלבד**.

**חלופה א' — SSO עם ספק זהויות חיצוני (דורש חבילת Elite):**
חיבור ה-SSO של Base44 ל-IdP ארגוני (Microsoft Entra ID / Google Workspace / Auth0) שאוכף MFA בכל התחברות. פתרון "נכון" ארכיטקטונית אך כרוך בעלות חבילת Elite + ניהול משתמשים ב-IdP. חלופה קלה יותר באותו כיוון: הפעלת Google/Microsoft authentication (מתג קיים) — מקבלים את ה-MFA של חשבון Google/Microsoft, אבל זה מותנה בכך שלכל המשתמשים יש חשבון כזה עם MFA מופעל, **ואינו מבטיח OTP בכל כניסה** (session נשמר).

**חלופה ב' — OTP ברמת האפליקציה (ממלא את הדרישה כלשונה — המומלץ):**
שכבת אימות שנייה בקוד שלנו, אחרי הלוגין של Base44 ולפני מתן גישה למסכים:
1. **ישות חדשה `MfaChallenge`** — user_id, code_hash (לא הקוד עצמו), expires_at (5 דק'), attempts, verified_at.
2. **פונקציית backend `sendMfaCode`** — מייצרת קוד 6 ספרות, שולחת ב-SMS דרך Twilio (התשתית קיימת — `sendSMS`), עם rate-limiting (המנגנון הקיים `_shared/rateLimit`).
3. **פונקציית backend `verifyMfaCode`** — אימות הקוד + סימון ה-session כמאומת. לפי דרישת אורטל — **בכל כניסה** (ללא "זכור מכשיר"); אם ירצו לרכך בהמשך, אפשר תוקף per-device.
4. **UI** — מסך ביניים אחרי לוגין עם רכיב `input-otp` הקיים; חסימת ניווט ב-`AuthProvider`/`RoleGuard` עד לאימות.
5. **אכיפה בצד שרת** — הפונקציות הרגישות (ניהול משתמשים, ניקוי נתונים, הרשאות) בודקות שה-session עבר MFA, לא רק ה-UI.
6. דרישות משלימות: מספר טלפון מאומת לכל משתמש (קיים שדה phone), נתיב שחזור למשתמש שאיבד גישה (admin משחרר), תיעוד ב-AuditLog.
*הערכת מאמץ: מספר ימי פיתוח + בדיקות. תלות: חשבון Twilio production פעיל (סעיף פתוח בפני עצמו — פרק 9.2).*

**המלצה:** מכיוון שהפלטפורמה אינה מציעה MFA מובנה והדרישה היא OTP בכל כניסה — **חלופה ב' היא המסלול המומלץ** (תלות: הפעלת Twilio production). חלופה א' (SSO/Elite) רלוונטית רק אם הארגון ממילא שוקל ניהול זהויות מרכזי. להחיל קודם על admin, ואז להרחיב לכלל התפקידים.
...
7. **מימוש/החלטה לגבי MFA** (דרישת אורטל, פרק 13.3) — הבירור מול הפלטפורמה כבר בוצע (אין MFA מובנה ב-Base44); יש להחליט בין חלופה א' (שדרוג ל-Elite עבור SSO) לבין חלופה ב' המומלצת (פיתוח OTP ב-SMS באפליקציה) ולהתחיל במימוש.
