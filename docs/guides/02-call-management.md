...
flowchart TD
    ...
    E -->|"הספק קיבל"| F["שובץ ספק / ספק בדרך"]
    E -->|"דחה / פג תוקף"| D
...
📌 **טיפ:** בכל שינוי סטטוס נוצרת רשומת היסטוריה (CallHistory), והסטטוס משתקף אוטומטית גם בתור העבודה (WorkQueue) ובתיק (Case) — אין צורך לעדכן בכמה מקומות.
...
- **קיבל** → הקריאה עוברת ל"ספק בדרך", הספק מסומן עסוק, והמוקד מקבל התראה.
...
במערכת 14 סטטוסים, אך במסך פרטי הקריאה מוצגים בכל רגע **רק המעברים המותרים מהשלב הנוכחי** — כך נמנעות טעויות ודילוגים לא חוקיים.
...
📌 **טיפ:** את המעברים "ספק בדרך" → "הגיע" → סיום מבצע בדרך כלל **הספק מהנייד**; המוקדן רואה את העדכונים בזמן אמת ויכול לעדכן במקומו במקרה הצורך (למשל ספק שמדווח בטלפון).
...
| ספק בדרך | הספק אישר ויצא לדרך | הספק (בקבלת ההצעה) | SMS ללקוח: "הספק בדרך אליך! צפי הגעה: {eta}"; הספק מסומן עסוק; התראה למוקד; מעקב GPS + עדכון ETA |
