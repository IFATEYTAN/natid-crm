...
### [2026-07-15] Feature: סנכרון דו-כיווני מול נתי — פונקציית דחיפה `pushNatiUpdates` + הגנות קונפליקט במשיכה

**בעיה:** הסנכרון מול נתי היה חד-כיווני (נתי → CRM בלבד). עם קבלת הרשאות כתיבה ל-DB של נתי, נדרש שהעבודה שנעשית ב-CRM (סגירת קריאות, שיבוץ ספקים, זמני הגעה/סיום, הערות מוקד, אישורי בקרת איכות) תשתקף חזרה בנתי. בנוסף, בלי הגנות בצד המשיכה, סנכרון דו-כיווני נכנס למרוץ: מוקדנית סוגרת קריאה ב-CRM ← המשיכה הבאה רואה שהקריאה עדיין פתוחה בנתי ומחזירה אותה לפתוחה ← הדחיפה משווה CRM==נתי==פתוח ולא כותבת כלום — הסגירה אבדה לחלוטין. מרוץ דומה: סטטוסים עדינים מקומיים (vendor_arrived, in_progress) נמחצו בכל משיכה חזרה ל-assigning כי לנתי יש רק דלי "בטיפול" אחד.
**פתרון:** (1) פונקציה חדשה `pushNatiUpdates` (מראה של `syncNatiData`: אותה שכבת חיבור inline עם TLS מוצמד, circuit breaker משותף ב-Deno KV, אימות admin/automation, cooldown 60ש', dry_run): שולפת קריאות שמקורן בנתי (call_number מספרי) שהשתנו מאז watermark אינקרמנטלי (KV, עם חפיפה של 10 דק'), משווה כל אחת מול הרשומה החיה ב-`call_open_appeals`, וכותבת רק הפרשים — עד 30 UPDATE לריצה. מדיניות שמרנית: סטטוס נע קדימה בלבד (0→1 בשיבוץ; פתוח→2/3 בסגירה/ביטול; לעולם לא פותחת מחדש ולא מורידה סטטוס); supplier_id/זמנים/הערות נכתבים **רק לשדות ריקים בנתי** (החלטת מוקדן של נתי תמיד גוברת); inspector_approves רק 0→1 ורק ל-QC ידני ב-CRM. תאריכים מומרים מ-ISO UTC לזמן ירושלים נאיבי (Intl, ההופכי של parseNatiDate). (2) הגנות במשיכה (`syncNatiData`): סטטוס טרמינלי מקומי (completed/cancelled) לא נדרס ע"י סטטוס פתוח מנתי (הדחיפה תסגור את נתי בריצה הבאה), וסטטוס נכנס שנופל לאותו "דלי נתי" כמו הקיים לא מחליף אותו (שומר על הרזולוציה המקומית). (3) UI בפאנל הסנכרון (`NatiSyncPanel`): מקטע "דחיפת עדכונים לנתי" עם Dry Run ופירוט שדות לכל קריאה. (4) הדחיפה עובדת על Call בלבד — Case הוא מראה לדוחות (ראו לקח מ-15/07 למטה) ומתעדכן ממילא במשיכה הבאה אחרי שנתי מתעדכנת.
**לקח:** סנכרון דו-כיווני בטוח = דחיפה מבוססת-diff מול הרשומה החיה (מונעת הד אינסופי משיכה↔דחיפה בלי לסמן רשומות) + כללי בעלות פר-שדה (מי גובר על מה) + הגנות סימטריות בצד המשיכה. בלי ההגנות במשיכה, הדחיפה לבדה מאבדת עדכונים במרוץ. חובה להגדיר את שתי הפונקציות ב-Scheduler של Base44 בהסטה (למשל sync בדקות 0,5,10 ו-push בדקות 2,7,12) כדי לקצר את חלון המרוץ.
**קבצים:** `base44/functions/pushNatiUpdates/entry.ts` (חדש), `base44/functions/syncNatiData/entry.ts`, `src/features/settings/api.js`, `src/features/settings/hooks/useNatiSync.js`, `src/components/integrations/NatiSyncPanel.jsx`, `docs/SCHEDULED_FUNCTIONS.md`, `SYSTEM_SPECIFICATION_v4.md`
...
**פתרון:** דפוס הגנה אחיד (inline בכל פונקציה, בלי מודול משותף — ראו לקח 09/07): פונקציות מתוזמנות/ידניות דורשות כותרת `x-internal-secret` התואמת ל-env var‏ `INTERNAL_JOB_SECRET` או משתמש מאומת (admin, וב-autoTransferAgentCalls כל משתמש מחובר). פונקציות המופעלות ע"י אוטומציית ישויות של הפלטפורמה (onNewCase, sendVendorAssignmentSMS במסלול אוטומציה) לא יכולות לדרוש אימות — במקומו: **אי-אמון מוחלט ב-payload** (שליפת הרשומה מחדש מה-DB לפי id בלבד) + דה-דופ (SMS לא יישלח שוב תוך 10 דקות לאותה קריאה). ctiWebhook עבר ל-fail-closed עם `CTI_WEBHOOK_SECRET` (כולל הנחיה במסך CTISettings). אגב כך תוקנו שאילתות `filter({})` שטענו את כל הטבלה לסינון בזיכרון.
**לקח:** פונקציה שמופעלת ע"י אוטומציית פלטפורמה אנונימית אינה ניתנת לנעילה באימות — ההגנה הנכונה היא re-fetch מה-DB לפי id (הקלט היחיד שנשאר הוא "עבד רשומה אמיתית") + אידמפוטנטיות/דה-דופ. ממצאי "exposed secret" על Deno.env.get בצד שרת הם לרוב שווא — הסכנה היא פונקציה לא מוגנת שמשתמשת בסוד, לא הסוד עצמו. נדרש: הגדרת `INTERNAL_JOB_SECRET` ו-`CTI_WEBHOOK_SECRET` ב-env של האפליקציה + הוספת הכותרת בקריאות מתוזמנות ובמרכזייה.
**קבצים:** `base44/functions/{checkAndSendNotifications,checkContractExpiry,checkDepositExpiry,autoTransferAgentCalls,onNewCase,sendVendorAssignmentSMS,ctiWebhook}/entry.ts`, `src/pages/CTISettings.jsx`

### [2026-07-13] Feature: סינון סטטוסים מדורג בקריאה — מציגים רק את הצעד הבא האפשרי

**בעיה:** בדיקות דורית (13/07, סעיף 10): אחרי שיבוץ ספק, בורר הסטטוס במסך הקריאה הציג את **כל 14 הסטטוסים** בבת אחת ("ספר במקום, ספק הגיע, סגירת קריאה ועוד מלא") — כולל מצבים שאינם רלוונטיים לשלב הנוכחי. גם תפריט הפעולות הציג תמיד את אותם שלושה קיצורי סטטוס קבועים. דרישת הלקוח: בכל שלב יופיע רק מה שניתן ללחוץ (שובץ ספק → "ספק הגיע ללקוח"+ביטול; הגיע ללקוח → אחסנה/יעד+ביטול; באחסנה → אחסנה נוספת/יעד סופי+ביטול; הגיע ליעד → סגירה/ביטול).
**פתרון:** נוסף `src/config/statusTransitions.js` — מפת מעברים (state machine) כמקור אמת יחיד למעברי סטטוס ידניים, עם תוויות הקשריות למעבר ("ספק הגיע ללקוח", "ספק הגיע לאחסנה", "סגירת קריאה", "ביטול פניה"). בורר הסטטוס ב-`CallDetails` מציג רק את המעברים המותרים מהסטטוס הנוכחי (הנוכחי מוצג כפריט disabled), ותפריט הפעולות (`CallActionsMenu`) מסנן את קיצורי הסטטוס, "סגור קריאה" ו"ביטול קריאה" לפי אותה מפה. בנוסף: בחירת "ביטול פניה" בבורר פותחת כעת את דיאלוג הביטול (כללי עירבון + סיבה) במקום כתיבת סטטוס ישירה שעקפה את הכללים. "ספק הגיע ליעד" אינו סטטוס נפרד — ממומש דרך "סגירת קריאה" (סטטוס סגירה `tow_done`), ואחסנה דרך `in_storage`; המשך לאחסנה נוספת דרך סגירה "גרר לאחסנה" או קריאת המשך.
**לקח:** בורר סטטוס גולמי שמציג את כל ה-enum מבלבל משתמשי קצה ומאפשר מעברים לא-חוקיים (כולל עקיפת דיאלוג הביטול). כשיש זרימה עסקית מוגדרת — לקודד אותה כמפת מעברים במקום אחד ולסנן את כל נקודות ה-UI ממנה. סטטוס לא ממופה לא נחסם (fallback להצגת הכל) כדי לא לנעול מצבים לא-צפויים.
**קבצים:** `src/config/statusTransitions.js` (חדש), `src/pages/CallDetails.jsx`, `src/components/call-details/CallActionsMenu.jsx`

### [2026-07-09] Bug: כל פונקציות ה-backend החזירו 404 — מודול `_shared` שטוח לא נפרס + סינון ספקים ריק

**בעיה:** בדיקות ה-QA של דורית (09/07) העלו שכל פעולה שנשענת על פונקציית backend נכשלה בבת אחת: שיבוץ ספק ("Request failed with status code 404: השיבוץ נכשל"), עדכון סטטוס ("שגיאה בעדכון סטטוס"), סגירת קריאה — "שגיאה חדשה" שלא הייתה קודם. שורש: התיקון מהבוקר (#174, "gate all backend functions on app-level role") הוסיף לכל 36 הפונקציות ייבוא `./_shared/appRole.ts` — אבל הקובץ נוצר **שטוח** (`_shared/appRole.ts`) במקום בקונבנציה המתועדת `_shared/<name>/entry.ts` (ראו לקח מ-06/07). מודול שלא בקונבנציה לא נפרס (כמו `_shared/natiDb` בזמנו), ה-import נכשל בטעינה, וכל פונקציה שמייבאת אותו מפסיקה להתקיים בפריסה → 404 על הכל. בנוסף (באג נפרד): סינון הספקים לפי "סוג רכב שירות" בדיאלוג השיבוץ הסתמך על `Vendor.service_type` בלבד — אבל כמעט לכל הספקים האמיתיים (מיובאים מהמערכת הישנה) השדה ריק `[]`; ברגע שנבחר סוג רכב, כל הספקים האמיתיים נעלמו מהרשימה ורק ספק הבדיקה E2E (שיש לו service_type מלא) נשאר — "בחרתי ניידת או גרר לשיבוץ ולא מאפשר לי".
**פתרון:** (1) **ניסיון ראשון:** `_shared/appRole.ts` הועבר ל-`_shared/appRole/entry.ts` (קונבנציית התיקייה) — **אך גם זה לא נפרס אמין**; פרסום ("Publish") מפרס רק את הפרונטאנד, וסנכרון הקוד מ-GitHub לא הפעיל redeploy של הפונקציות. (2) **הפתרון הסופי שעבד:** לוגיקת `resolveAppRole` **הוטמעה (inline) בתוך כל אחת מ-36 הפונקציות** (בלוק "Inline app-role resolution" ב-`entry.ts` של כל פונקציה) — כך אף פונקציה אינה תלויה במודול משותף חדש, וכל `entry.ts` נפרס בנפרד. הקובץ `_shared/appRole.ts` הושאר כ-stub deprecated (עם `Deno.serve` שמחזיר 404) כדי לשמור על רשומת הפריסה תקינה. (3) `AssignVendorDialog`: סיווג ניידת/גרר לפי `service_type` אם קיים, אחרת **לפי שם הספק**; ספק לא-מסווג מוצג בשתי הרשימות. (4) מולאו `service_type` לספקים אמיתיים בנתוני production (23 ספקים: 18 גרירה, 4 ניידת, 1 שמשות).
**לקח:** שלושה לקחים. (א) **מודול `_shared` *חדש* עלול לא להירשם לפריסה בפלטפורמה הזו** — גם בקונבנציית התיקייה. כשמוסיפים לוגיקה משותפת לפונקציות קיימות רבות, בטוח יותר **להטמיע (inline)** מאשר להסתמך על מודול משותף חדש. (ב) **"Publish" מפרס פרונטאנד בלבד;** פונקציית backend נפרסת מחדש רק כשקובץ ה-`entry.ts` שלה עצמה נערך דרך צינור העריכה של Base44 — סנכרון מ-GitHub לבד לא מספיק. (ג) **אימות פריסה חייב להיעשות עם משתמש *מוזמן*, לא הבעלים:** הבעלים הוא admin-פלטפורמה אמיתי ולעולם לא משחזר את הבאג (עובר כל בדיקת תפקיד); משתמש מוזמן נושא role=`user` ומקבל 403/404. בדיקת העשן: מוקדן מוזמן ← detectSmartAlerts מחזיר **200** (ולא 403/404). שינוי רוחבי חייב בדיקת עשן כזו על פונקציה אחת מיד אחרי הפריסה. וגם: פיצ'ר סינון חדש חייב בדיקה מול **נתוני אמת** — סכמה עם שדה ≠ נתונים מולאו.
**קבצים:** כל 36 קבצי `base44/functions/*/entry.ts` (בלוק inline app-role), `base44/functions/_shared/appRole/entry.ts` (stub deprecated), `src/components/calls/AssignVendorDialog.jsx`; `.claude/skills/e2e-testing.md` + `scripts/qa/*` (סקריפטי אימות פריסה ובדיקות E2E עם 3 המשתמשים)
### [2026-06-17] Feature: סינון ספקים בדיאלוג השיבוץ לפי סוג רכב ואזור

**בעיה:** (דרישת דורית, סעיף 6) בבחירת ספק ידנית הוצגו כל הספקים הזמינים ללא קשר לבחירה — צריך שיוצגו רק ספקים שמתאימים לסוג הרכב שנבחר (גרר/ניידת) ולאזור הקריאה בארץ.
**פתרון:** הסינון מבוסס על שני שדות שכבר קיימים בכרטיס הספק ב-CRM (ישות `Vendor`): `service_type` (ערכים: `tow_truck`, `mechanic`, `tire_service`, `fuel_delivery`, `locksmith`, `multi_service`) ו-`coverage_areas` (6 אזורים: center/sharon/north/south/jerusalem/lowlands). מיפוי `dispatch_type` → סוגי שירות: ניידת ⇐ mechanic/tire_service/fuel_delivery/locksmith/multi_service; גרר/גרר+אחסנה ⇐ tow_truck/multi_service. אזור הקריאה מנורמל לאזור הראשי (הרישא שלפני הקו התחתון, למשל `north_haifa` ⇐ `north`) כדי להתאים לאזורי הספק הגסים. הסינון "סלחני לחוסר מידע" — ספק נפסל רק אם מוגדר לו מידע שסותר את הבחירה — ויש מתג "הצג את כל הספקים" + מסלול מילוט כשהרשימה מתרוקנת. הסינון עקבי עם מנגנון ההתאמה ב-`autoAssignVendor`.
**לקח:** איכות הסינון תלויה ישירות במילוי `service_type` ו-`coverage_areas` בכרטיסי הספקים ב-CRM. לכן הסינון לא מסתיר ספקים עם כרטיס חלקי, ותמיד יש דרך לראות את כולם — כדי שלא ניתקע בגלל נתונים חסרים.
**קבצים:** `src/components/calls/AssignVendorDialog.jsx`
...
1. **`syncNatiData` לעולם לא מנקז צבר ישן.** הפונקציה מעבדת 30 קריאות לריצה, אבל בחרה תמיד את "30 המעודכנות/חדשות ביותר" לפי `has_updated` ואז `date_added` — בלי עדיפות לקריאות שטרם קיימות ב-CRM כלל. עם ~155 קריאות פתוחות אצל נתי, קריאות ישנות שלא התעדכנו מעולם לא הגיעו לתור העיבוד. בנוסף, לא הייתה הגדרת אוטומציה מתוזמנת ל-`syncNatiData` כלל — `docs/SCHEDULED_FUNCTIONS.md` פשוט לא הזכיר אותה.
2. **`detectSmartAlerts` קרסה ל-500/502 בדשבורד.** שלוש שליפות ללא הגבלה/מיון (`CallAssignmentAttempt`, `Case` במצב `completed`, ו-`Notification`) שלפו טבלאות שלמות — עם נפח הנתונים שגדל אחרי שסנכרון נתי החל למלא את המערכת בקריאות אמיתיות, זה חרג מזמן/זיכרון. גם שגיאת ה-catch הוחלפה בהודעה גנרית ("Failed to detect alerts") שהסתירה את הסיבה האמיתית. וגם: כל קריאה לא-משובצת מעל 15 דקות יצרה התראה נפרדת לכל אדמין/מוקדן — עם צבר אמיתי (ולא רק דמו) זה הפך לספאם.
3. **`KnowledgeBase.jsx` קרא לישות `KnowledgeArticle` שלא הוגדרה בסכמה כלל** (`base44/entities/`) — למרות שהיא מופיעה בניווט, בהרשאות (`permissions.js`), ובנתוני הדמו (`demoData.js`). כל טעינה של הדף נכשלה ב-404 על ה-entity.

**פתרון:**
1. `syncNatiData`: לוגיקת המיון עברה לשלושה שלבים — קודם קריאות שה-`call_number` שלהן עדיין לא קיים ב-CRM (נטען מראש `Call.filter({})` *לפני* הבחירה, ולא רק אחריה כמו קודם), אחר כך `has_updated===1`, ואז `date_added` יורד. הפעולה משתמשת מחדש באותה שליפה גם בהמשך (`existingCalls`) כדי לא לכפול קריאת DB. תועד `syncNatiData` ב-`docs/SCHEDULED_FUNCTIONS.md` עם המלצת תדירות (5–10 דקות) ותמיכה ב-`automation_key`.
2. `detectSmartAlerts`: כל שליפה שהייתה ללא הגבלה קיבלה מיון+`limit` (`'-created_date'`/`'-completed_at'`, 500–2000 רשומות). ה-catch הכללי מחזיר כעת את `error.message` האמיתי במקום מחרוזת קבועה (הפונקציה גם ככה דורשת admin/operator/automation-key, כך שאין דליפת מידע לצד לא-מאומת). תרחיש "קריאה ללא שיבוץ" רוכז לכדי **התראה אחת** כשיש כמה קריאות חורגות (במקום אחת לכל קריאה), ונוסף תקרה כללית (`MAX_ALERTS_PER_RUN=10`) כרשת ביטחון על פני כל התרחישים.
3. נוצרה `base44/entities/KnowledgeArticle.jsonc` (title/category/problem_description/solution/tags) בהתאם לשדות שכבר צפויים ב-`KnowledgeBase.jsx` וב-`demoData.js`.

**לקח:** "עיבוד 30 האחרונות" ו"עיבוד 30 החדשות ביותר" הם שני דברים שונים — בלי בדיקת קיום מפורשת מול המערכת המקומית, צבר ישן לעולם לא זוכה לתור. שליפת טבלה שלמה בלי `limit`/מיון היא פצצת-שהיות רדומה שרק "מתפוצצת" כשנפח הנתונים גדל בפועל (בדיוק כשסנכרון תקין מתחיל לעבוד!) — לכתוב `limit` מהיום הראשון גם אם הטבלה קטנה כרגע. הודעת שגיאה גנרית בפונקציית מערכת פנימית (מוגנת auth) מסתירה בדיוק את המידע שצריך לאבחן תקלה — סינון הודעות שגיאה שייך לגבולות הרשאה חיצוניים, לא לכל endpoint. ולבסוף: כשדף/הרשאה/דאטה-דמו כבר "מדברים" על ישות מסוימת, אבל אין schema בפועל — זו לא תקלת קוד, זו סכמה חסרה; לחפש קודם בתיקיית `base44/entities/` לפני שמניחים שהבאג הוא בקוד ה-React.
**קבצים:** `base44/functions/syncNatiData/entry.ts`, `base44/functions/detectSmartAlerts/entry.ts`, `base44/entities/KnowledgeArticle.jsonc`, `docs/SCHEDULED_FUNCTIONS.md`

---

### [2026-07-06] Architecture: קריאות שנפתחות בזרימה החיה היו בלתי-נראות לדוחות ולהתראות חכמות

**בעיה:** תוך כדי הביקורת על תהליך ניהול הקריאה התגלה שהמסך ליצירת קריאה (`NewCase.jsx`) והבוט (`99digitalBot`) יוצרים רק `Call`+`WorkQueue` — לא `Case`. הערת קוד ב-`NewCase.jsx` מסבירה שזה היה מכוון: "יצירת Case כאן גרמה לכך שפתיחות חדשות לא הגיעו לצד הספק" (פורטל ספק/GPS/שיבוץ צורכים את `Call`, לא את `Case`). אבל `Reports.jsx`, `ServiceProviders.jsx`, ו-`detectSmartAlerts` (שתוקנה אתמול) כולן שולפות מ-`Case` — כך שקריאות שנפתחות בשתי הזרימות החיות היחידות היו, בפועל, בלתי נראות לגמרי בדוחות ולמנוע ההתראות. הענף שממפה `Case` ב-`syncCallStatus` היה חי רק בזכות סנכרון נתי (שיוצר Case תואם) ושני טפסים ישנים יותר (`SpecialCaseForm`, `PrivateService`).
**פתרון:** במקום להעביר את שלושת הצרכנים (סיכון גבוה — Reports/ServiceProviders מכילות לוגיקת דיווח נרחבת מבוססת שדות Case) — הוחלט להשלים את הפער בנקודת המקור: גם `NewCase.jsx` וגם `99digitalBot` יוצרים כעת **גם** `Case` תואם (best-effort, לא חוסם — כישלון ביצירת ה-Case לא פוסל את הקריאה/התור), עם `case_number === call_number` (אותה מוסכמת קישור כמו בסנכרון נתי). `Call` נשאר מקור האמת התפעולי; `Case` הוא כעת מראה קריאה לצורך דיווח/התראות בלבד, בעקביות בין כל 4 מקורות היצירה (NewCase, בוט, Nati sync, טפסים ייעודיים).
**לקח:** הערת קוד שמסבירה "למה עשינו X ולא Y" (כאן: Call bukan Case) לא בודקת אוטומטית שאף אחד לא עדיין תלוי ב-Y — יש לחפש את כל הצרכנים של הישות הישנה (`grep` על שם הישות) לפני שמניחים שהמעבר הושלם. כשיש שני "מקורות אמת" מקבילים לאותה ישות עסקית (Call לתפעול, Case לדיווח) — כל נתיב יצירה חדש חייב ליצור את שניהם, אחרת חלק מהמערכת "לא רואה" חלק מהנתונים בשקט (בלי שגיאה גלויה).
**קבצים:** `src/pages/NewCase.jsx`, `base44/functions/99digitalBot/entry.ts`
