...
**פתרון:** (1) `_shared/appRole.ts` הועבר ל-`_shared/appRole/entry.ts` — נתיב הייבוא `./_shared/appRole.ts` נשאר זהה, בהתאם לקונבנציה שעובדת ב-5 המודולים המשותפים האחרים. (2) `AssignVendorDialog`: הסיווג ניידת/גרר נעשה לפי `service_type` אם קיים, אחרת **לפי שם הספק** (גרר/גורר/גרירה ↔ ניידת/סטרטר); ספק שלא ניתן לסווג כלל מוצג בשתי הרשימות — עדיף להציג יותר מדי מאשר לחסום שיבוץ.
**לקח:** הקונבנציה `_shared/<name>/entry.ts` היא לא העדפה סגנונית אלא תנאי פריסה — קובץ `.ts` שטוח תחת `_shared/` נראה תקין בקוד, עובר lint ו-build, ונכשל רק בזמן ריצה בפרודקשן עם 404 גורף על כל הפונקציות שמייבאות אותו. שינוי רוחבי שנוגע בכל הפונקציות (כמו #174) חייב בדיקת עשן על פונקציה אחת בסביבה החיה מיד אחרי הפריסה. וגם: פיצ'ר סינון חדש חייב להיבדק מול **נתוני אמת** — סכמה שמגדירה שדה לא אומרת שהנתונים מולאו.
**קבצים:** `base44/functions/_shared/appRole/entry.ts` (הועבר מ-`_shared/appRole.ts`), `src/components/calls/AssignVendorDialog.jsx`
